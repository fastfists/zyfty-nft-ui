...
export class TokenService {
  ...
  constructor(
    ...
    @Inject('tokenAddress') @Optional() public tokenAddress?: string
  ) {
  ...
  async mintMore() {
    ...
    let test_token = new ethers.Contract(
      this.address,
      TestToken.abi,
      this.signer$.value!
    );
    await test_token.test_mint(
      this.provider.account.value,
      ethers.utils.parseUnits('50', await this.token.decimals())
    );
  }
    ...
    await this.provider.ethereum.request({
      ...
      params: {
        ...
        options: {
          ...
          decimals: await this.decimals(), // The number of decimals in the tokenAddress
        },
      },
    });
  }
  ...
  async decimals(): Promise<string> {
    ...
    return await this.token.decimals();
  }
